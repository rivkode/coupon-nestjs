...
`test/envelope.e2e-spec.ts` 는 세 서버의 에러 매핑 차이까지 고정한다 —
특히 **server-b 만 다른 두 지점**(`INTERNAL_STATE` 500, `MISSING_HEADER` 핸들러 부재)은
원본의 의도된 차이이므로 통일하지 말 것.
...
- [ ] 2단계 — `server-c`: 엔티티 → 리포지토리 → 발급 트랜잭션(비관락) → redeem(낙관락) → Outbox → 이벤트 캐시 → Kafka consumer
